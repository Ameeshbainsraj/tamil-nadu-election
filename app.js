...
function render(data) {
  ...
  Object.keys(parties).forEach(key => {
    ...
    const needsText = key === 'others' ? `${p.seats} seats total` : getNeedsText(p.seats);
  ...
  if (leader) {
    ...
    document.getElementById('leading-sub').textContent = `${leader.name} · ${leader.seats} seats`;
  }
...
async function fetchData() {
  try {
    const url = `https://opensheet.elk.sh/1170HdchakPRIZO4URqot2jfAhtkorl1tP9-Tpq80Ia0/Sheet1`;
